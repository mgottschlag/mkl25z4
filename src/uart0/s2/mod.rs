...
pub enum RAFR {
    #[doc = "UART receiver idle waiting for a start bit."]
    _0,
    #[doc = "UART receiver active ( UART _RXD input not idle)."]
    _1,
}
impl RAFR {
...
pub enum MSBFR {
    ...
    _0,
    #[doc = "MSB (bit9, bit8, bit7 or bit6) is the first bit that is transmitted following the start bit depending on the setting of C1[M], C1[PE] and C4[M10]. Further, the first bit received after the start bit is identified as bit9, bit8, bit7 or bit6 depending on the setting of C1[M] and C1[PE]."]
    _1,
}
impl MSBFR {
    #[doc = r" Returns `true` if the bit is clear (0)"]
